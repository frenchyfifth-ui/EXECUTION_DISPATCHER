...
def load_normalized_payloads() -> List[ExecutionEvent]:
    """Load normalized JSON payloads from disk."""
    payloads: List[ExecutionEvent] = []
    for payload_path in sorted(NORMALIZED_DIR.glob("*.json")):
        with payload_path.open("r", encoding="utf-8") as handle:
            payloads.append(ExecutionEvent.from_dict(json.load(handle)))
    return payloads
...
def iter_target_platforms(payload: ExecutionEvent) -> Iterable[str]:
    """Yield normalized platform names."""
    return [platform.upper() for platform in payload.target_platforms]
...
def dispatch_all() -> None:
    ...
    live_mode = os.getenv("DISPATCH_LIVE", "false").lower() in {"1", "true", "yes"}
    if not live_mode:
    ...
    for payload in payloads:
        ...
        for platform in iter_target_platforms(payload):
            ...
            adapter = adapter_class(credentials=credentials, logger=logger, live_mode=live_mode)
